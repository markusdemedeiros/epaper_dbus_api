...
DIR_FONTS    = ./lib/Fonts
DIR_GUI      = ./lib/GUI
DIR_QUEUE	 = ./lib/StsQueue
DIR_Src 	 = ./src
DIR_BIN      = ./bin

OBJ_C = $(wildcard ${DIR_EPD}/*.c ${DIR_GUI}/*.c ${DIR_Src}/*.c ${DIR_FONTS}/*.c ${DIR_QUEUE}/*.c)
...
TARGET = epd
CC = gcc
MSG = -g -O0  -Wall -lpthread `pkg-config --cflags dbus-1` `pkg-config --libs dbus-1`
CFLAGS += $(MSG)
...
${DIR_BIN}/%.o:$(DIR_QUEUE)/%.c
	$(CC) $(CFLAGS) -c  $< -o $@ $(DEBUG)

${DIR_BIN}/%.o:$(DIR_Src)/%.c
	$(CC) $(CFLAGS) -c  $< -o $@ -I $(DIR_QUEUE) -I $(DIR_Config) -I $(DIR_GUI) -I $(DIR_EPD) $(DEBUG)
